...
### 서브모듈(hwplib) 빌드 시 `target/` 디렉터리 변경 사항 처리

원본 Java 프로젝트(`hwplib`)를 빌드하면 서브모듈 디렉터리 아래에 `target/` 디렉터리가 생성되며, 이 산출물이 상위 Git 리포지터리(VSCode의 Source Control 등)에 변경 사항으로 잡히는 경우가 있습니다.

로컬에서 이를 깔끔하게 숨기고 싶다면, 서브모듈 디렉터리(`hwplib/`)에 다음과 같은 `.gitignore`를 추가하면 됩니다.
...
> ⚠️ **중요**: 위 `.gitignore` 추가는 **본 포팅 프로젝트의 작업 환경 편의**를 위한 로컬 설정입니다. 원본 프로젝트([neolord0/hwplib](https://github.com/neolord0/hwplib))의 빌드/리포지터리 운용 정책과 무관하므로, 이 변경 사항을 원본 리포지터리에 PR로 제출하지 말아 주십시오. 원작자의 의사결정 영역을 침해하지 않도록 협조 부탁드립니다.
